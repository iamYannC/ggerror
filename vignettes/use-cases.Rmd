...
This vignette demonstrates more advanced usage of the package. For a "get-started" and overview of this package's basic usecase, see `vignette("ggerror")`.
...
Style improvement: Beneath ##SETUP, add callout-info class that is collapsible, and collapsed by default, with the data info
and more from `airquality` docs. Offer the users to expand the following
info box for more information, or advise them to just type `?airquality`
for more information. vignette should support collapsible HTML (it is
rendered to a pkgdown website, im not mistaken - verify!).
...
# What happens if you add stat_summary(geom='point')? What central measure do the points represent?
```
...
\> NOTE FROM HUMAN TO LLM \>
If possible, add another collipsable cell with this code:
