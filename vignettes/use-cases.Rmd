...
- **Flexible styling**: `ggerror` provides us with per-direction styling
  ...
  differently, to show the direction the bar extends, rather than the sign of the error.
  Experiment with this by tweaking either the actual color value,
...
`stat_error()` follows ggplot2's `fun.data` contract: pass a summary function
...
\[
\mathrm{md}(x) = \mathrm{median}(x), \qquad
\mathrm{mae}(x) = \frac{1}{n}\sum_{i=1}^{n}\left|x_i - \mathrm{md}(x)\right|
\]

\[
y = \mathrm{md}(x), \qquad
y_{\min} = \mathrm{md}(x) - \mathrm{mae}(x), \qquad
y_{\max} = \mathrm{md}(x) + \mathrm{mae}(x)
\]
