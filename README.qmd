...
[![CRAN-status](https://www.r-pkg.org/badges/version/ggerror?cache=1)](https://CRAN.R-project.org/package=ggerror) [![R-CMD-check](https://github.com/iamYannC/ggerror/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/iamYannC/ggerror/actions/workflows/R-CMD-check.yaml) [![Codecov-coverage](https://codecov.io/gh/iamYannC/ggerror/graph/badge.svg)](https://app.codecov.io/gh/iamYannC/ggerror)
...
- Per-side styling through `colour_neg`, `width_pos`, `linetype_neg`, and more
...
```{r defaults}
#| code-fold: true
#| code-summary: "defaults and data prep"
library(ggplot2)
library(ggerror)
...
If you want a specific error geom (4 available, [see below](#supported-geoms)), either pick it with `error_geom = ...` or use the pinned wrappers.

```{r pointrange-wrapper}
ggplot(aq_monthly_avg, aes(Temp.mean, Month)) +
...
Custom summary functions work too, as long as they return data frames of one row with `y`, `ymin`, and `ymax` columns:

```{r custom-summary-plot}
...
## Supported geoms {#supported-geoms}
...
- [`vignette("ggerror")`](https://iamyannc.github.io/ggerror/articles/ggerror.html): simple defaults, asymmetric bars, one-sided bars
- [`vignette("use-cases")`](https://iamyannc.github.io/ggerror/articles/use-cases.html): `stat_error()`, custom summaries, and `sign_aware`
